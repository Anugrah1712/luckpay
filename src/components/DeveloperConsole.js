...
const DeveloperConsole = ({ closeConsole, openChatbot }) => {
  ...
  const handlePreprocessing = async () => {
    ...
    try {
      ...
      const response = await fetch("https://13.60.34.232:8000/preprocess", {
        method: "POST",
  ...
  const handleFinalConfiguration = async () => {
    ...
    try {
      ...
      const vectordbResponse = await fetch("https://13.60.34.232:8000/select_vectordb", {
        method: "POST",
        mode: "cors",  // Ensure CORS mode is enabled
      ...
      // Select chat model
      const chatModelResponse = await fetch("https://13.60.34.232:8000/select_chat_model", {
        method: "POST",
        mode: "cors",  // Ensure CORS mode is enabled
        headers: { "Content-Type": "application/x-www-form-urlencoded" },
